...
if __name__ == "__main__":
    ...
    scn_model_name = "BCG_cropped_ahr_SCN_val_fold3"
    ...
    state_dict_scn = torch.load(
        "../results/lightning_model_checkpoints/BCG_cropped/BCG_cropped_ahr_SCN_val_fold3.ckpt",
        map_location="cpu",
    )["state_dict"]
    ...
    scn_model_ensemble = AdaptiveHeatmapRegression(
        ...
        ensemble=True,
        ensemble_ckpts=glob(
            "../results/lightning_model_checkpoints/BCG_cropped/ensemble/ahr_SCN/*.ckpt"
        ),
        final_activation=torch.nn.ReLU(),
    )
    ...
    for i, (model, model_name) in enumerate(
        ...
        for pixel_based_sampling in pixel_based_sampling_options:
            ...
            for decoder_method in ["local_soft_argmax"]:
                ...
                save_path = (
                    f"../results/bcg_cropped_scn_evaluation_{model_name}_{decoder_method}.csv"
                )
                df_results.to_csv(save_path)
    # Save all results (pickle)
    all_results["model_filename"] = scn_model_name
    save_path = f"../results/bcg_cropped_scn_evaluation_det_results.pkl"
