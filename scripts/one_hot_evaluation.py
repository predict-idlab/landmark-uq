...
def perform_uncertainty_eval(
    ...
    if spatial_dims == 2:
        pred_heatmap_sigmas_val = pred_heatmap_cov_val[..., [0, 1], [0, 1]]
        pred_heatmap_sigmas_test = pred_heatmap_cov_test[..., [0, 1], [0, 1]]
    else:
        pred_heatmap_sigmas_val = pred_heatmap_cov_val[..., [0, 1, 2], [0, 1, 2]]
        pred_heatmap_sigmas_test = pred_heatmap_cov_test[..., [0, 1, 2], [0, 1, 2]]
    ...
    if "covariance_MC_orig_size" in val_results[0].keys():
        ...
        if spatial_dims == 2:
            pred_mc_sigmas_val = pred_mc_cov_val[..., [0, 1], [0, 1]]
            pred_mc_sigmas_test = pred_mc_cov_test[..., [0, 1], [0, 1]]
        else:
            pred_mc_sigmas_val = pred_mc_cov_val[..., [0, 1, 2], [0, 1, 2]]
            pred_mc_sigmas_test = pred_mc_cov_test[..., [0, 1, 2], [0, 1, 2]]
    ...
    cp_bonferroni_heatmap_sigma.fit(
        pred=pred_landmarks_val,
        sigmas=pred_heatmap_sigmas_val,
        target=true_landmarks_val,
    )
    for confidence_level in confidence_levels:
        df_results_temp = pd.DataFrame(columns=df_results.columns)
        ...
        in_region, area, error = cp_bonferroni_heatmap_sigma.evaluate(
            pred=pred_landmarks_test,
            sigmas=pred_heatmap_sigmas_test,
            target=true_landmarks_test,
            spacing=test_dataset.pixel_spacings.numpy(),
            confidence=confidence_level,
            return_summary=False,
    ...
    mr2ccp = MR2CCP(
        spatial_dims=spatial_dims, nb_landmarks=val_dataset.nb_landmarks, heatmap_size=heatmap_size
    )
    mr2ccp.fit(
    ...
    for confidence_level in confidence_levels:
        mr2c2r_aps = MR2C2R(
            ...
            nb_landmarks=val_dataset.nb_landmarks,
            aps=True,
    ...
    print("Fit and evaluate contour hugging regions with temperature scaling")
    deepcopy_model = deepcopy(model)
    deepcopy_model.temperature_scaling = True
    deepcopy_model.to("cuda")
    deepcopy_model.set_temperature(val_loader)
    ts_test_results = trainer.predict(deepcopy_model, test_loader)
    contour_hugging_ts = ContourHuggingRegressor(
        spatial_dims=spatial_dims, nb_landmarks=val_dataset.nb_landmarks, heatmap_size=heatmap_size
    )
    pred_heatmap_ts = np.concatenate([result["heatmaps"] for result in ts_test_results])
    pred_landmarks_ts = np.concatenate(
        [result["landmarks_orig_size"] for result in ts_test_results]
    )
    pred_padding_ts = np.concatenate([result["padding"] for result in ts_test_results])
    pred_original_dims_ts = np.concatenate([result["original_dims"] for result in ts_test_results])
    for confidence_level in confidence_levels:
        df_results_temp = pd.DataFrame(columns=df_results.columns)
        df_results_temp["idx"] = np.arange(len(test_dataset))
        df_results_temp["approach"] = "contour_hugging_ts"
        df_results_temp["confidence"] = confidence_level
        df_results_temp["model"] = model_name

        in_region, area, error = contour_hugging_ts.evaluate(
            heatmaps=pred_heatmap_ts,
            pred_landmarks=pred_landmarks_ts,
            target=true_landmarks_test,
            original_dims=pred_original_dims_ts,
            padding=pred_padding_ts,
            spacing=test_dataset.pixel_spacings.numpy(),
            confidence=confidence_level,
            return_summary=False,
        )
