...
if __name__ == "__main__":
    ...
    one_hot_model_name = "BCG_cropped_shr_unet_NLLLoss_test_shuffle"
    ...
    state_dict_one_hot = torch.load(
        "../results/lightning_model_checkpoints/BCG_cropped_test_shuffle/BCG_cropped_shr_unet_NLLLoss_test_shuffle.ckpt",
        map_location="cpu",
    )["state_dict"]
    ...
    one_hot_heatmap_model_ensemble = StaticHeatmapRegression(
        ...
        decoder_method=one_hot_decoder_method,
        ensemble_ckpts=glob(
            "../results/lightning_model_checkpoints/BCG_cropped_test_shuffle/ensemble/shr_unet/*.ckpt"
        ),
    )
    ...
    for i, (model, model_name) in enumerate(
        ...
        for pixel_based_sampling in pixel_based_sampling_options:
            ...
            for decoder_method in ["local_soft_argmax", "weighted_spatial_mean"]:
                ...
                save_path = f"../results/bcg_cropped_test_shuffle_one_hot_evaluation_{model_name}_{decoder_method}.csv"
                df_results.to_csv(save_path)
    # Save all results (pickle)
    all_results["model_filename"] = one_hot_model_name
    save_path = f"../results/bcg_cropped_test_shuffle_one_hot_evaluation_det_results.pkl"
