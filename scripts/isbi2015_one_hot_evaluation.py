...
if __name__ == "__main__":
    ...
    one_hot_model_name = "ISBI2015_shr_unet_NLLLoss_val_fold2_test_fold3"
    ...
    state_dict_one_hot = torch.load(
        "../results/lightning_model_checkpoints/ISBI2015/ISBI2015_shr_unet_NLLLoss_val_fold2_test_fold3.ckpt",
        map_location="cpu",
    )["state_dict"]
    ...
    one_hot_heatmap_model_ensemble = StaticHeatmapRegression(
        ...
        decoder_method=one_hot_decoder_method,
        ensemble_ckpts=glob(
            "../results/lightning_model_checkpoints/ISBI2015/ensemble/shr_unet/*.ckpt"
        ),
    )
    ...
    all_results = {}
    all_results["dataset"] = "ISBI2015"
    all_results["val_fold"] = 2
    all_results["test_fold"] = 3
    for i, (model, model_name) in enumerate(
        ...
        for pixel_based_sampling in pixel_based_sampling_options:
            ...
            for decoder_method in ["local_soft_argmax", "weighted_spatial_mean"]:
                ...
                save_path = (
                    f"../results/isbi2015_one_hot_evaluation_{model_name}_{decoder_method}.csv"
                )
                df_results.to_csv(save_path)
    # Save all results (pickle)
    all_results["model_filename"] = one_hot_model_name
    save_path = f"../results/isbi2015_one_hot_evaluation_det_results.pkl"
